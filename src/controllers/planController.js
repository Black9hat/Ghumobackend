 ...
 * Body: { planName, planType, commissionRate, noCommission, perRideIncentive, platformFeeFlat, platformFeePercent, monthlyFee, description, benefits }
 */
...
const {
  ...
  noCommission,
  perRideIncentive: perRideIncentive ?? 0,
  platformFeeFlat:  platformFeeFlat  ?? 0,
  platformFeePercent: platformFeePercent ?? 0,
  monthlyFee: monthlyFee || 0,
  planPrice: planPrice ?? monthlyFee ?? 0,
...
export const calculateEarningWithPlan = async (driverId, baseEarning) => {
  try {
    ...
    if (!driverPlan) {
      // No active plan — caller should use CommissionSetting instead.
      // Return a sentinel so tripController knows to fall back.
      return {
        finalEarning: null,
        baseEarning,
        commission: null,
    ...
    const commissionRate = driverPlan.noCommission ? 0 : driverPlan.commissionRate;
    const commission     = Math.round(baseEarning * commissionRate / 100 * 100) / 100;
    const finalEarning   = Math.round((baseEarning - commission) * 100) / 100;
