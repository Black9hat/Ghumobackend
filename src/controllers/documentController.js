...
const resolveDriverDocPath = (storedPath) => {
  ...
  try {
    ...
    res.status(200).json({
      driver: {
        ...
        // ✅ Vehicle fields — all included so Flutter profile page shows correct values
        // vehicleType is always from DB (admin may have set premium/car after onboarding)
        vehicleType: driver.vehicleType || "",
        vehicleModel: driver.vehicleModel || null,     // ✅ e.g. "Swift", "Honda City"
        vehicleBrand: driver.vehicleBrand || null,
        vehicleNumber: driver.vehicleNumber || null,
        seats: driver.seats ?? null,                   // ✅ seat count — 4=car/premium, 6=xl
...
export const uploadDriverDocument = async (req, res) => {
  try {
    ...
    // Also match legacy rows where side was missing/null/single for front uploads.
    const legacySideMatchers =
      side === "front"
        ? [{ side: "front" }, { side: "single" }, { side: null }, { side: { $exists: false } }]
        : [{ side }];
    ...
    // Keep only one active row per (user, docType, vehicleType, side).
    // Older duplicates can keep "rejected" and hide fresh re-uploads in admin views.
    ...
    const duplicateResult = await DriverDoc.updateMany(duplicateFilter, {
      $set: {
        imageDeleted: true,
    ...
    if (duplicateResult.modifiedCount > 0) {
      console.log(
        `🧹 Soft-deleted ${duplicateResult.modifiedCount} stale duplicate doc(s) for ${docTypeNormalized}/${side}`,
      );
    }
    ...
    const docsWithUrls = docs.map((doc) => {
      ...
      if (doc.url) {
        imageUrl = doc.url.startsWith("http")
          ? doc.url
          : `${baseUrl}/api/driver/document-image/${doc._id}`;
      }
...
export const getDriverDocumentImage = async (req, res) => {
  try {
    ...
    if (!doc || !doc.url || doc.imageDeleted) {
      return res.status(404).json({ message: "Document image not found" });
    }

    // Ensure drivers can only fetch their own document images.
    if (!req.user || !req.user._id || String(doc.userId) !== String(req.user._id)) {
      return res.status(403).json({ message: "Forbidden" });
    }

    const filePath = resolveDriverDocPath(doc.url);
    if (!filePath || !fs.existsSync(filePath)) {
      return res.status(404).json({ message: "Document image not found on storage" });
    }

    return res.sendFile(filePath);
  } catch (err) {
    console.error("❌ Error serving driver document image:", err);
    return res.status(500).json({ message: "Server error", error: err.message });
  }
};
