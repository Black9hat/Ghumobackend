...
export const createRazorpayPlanOrder = async (req, res) => {
  ...
  try {
    ...
    // ── Check if driver already has active plan (ONE PLAN AT A TIME) ──
...
export const verifyPlanPayment = async (req, res) => {
  ...
  try {
    ...
    // ── Emit socket notification with full plan details ──
    // Send to both individual driver room and all drivers (for real-time updates)
    safeEmit(req.io, `driver_${driverId}`, 'plan:activated', {
      ...
      message: `✅ Plan "${plan.planName}" activated! Commission: ${plan.noCommission ? '0%' : plan.commissionRate + '%'} | Bonus: ${plan.bonusMultiplier}x`,
