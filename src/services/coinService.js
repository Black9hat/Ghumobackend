...
export async function getCustomerCoinSummary(customerId) {
  ...
  return {
    ...
    // ✅ FORMAT TRANSACTIONS FOR DISPLAY (both earn and spend)
    transactions: transactions.map(tx => ({
      type:        tx.coinsEarned > 0 ? 'earn' : 'spend',  // Determines icon/color
      isEarned:    tx.coinsEarned > 0,
      coins:       Math.abs(tx.coinsEarned),  // Show positive number in UI
      description: tx.description,
      createdAt:   tx.createdAt,
      breakdown:   tx.breakdown,
      balanceAfter: tx.balanceAfter,  // Show balance after transaction
    })),
    // Legacy fields
    totalCoins:    coins,
