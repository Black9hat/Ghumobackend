...
router.post("/update-fcm", async (req, res) => {
  try {
    const { phone, fcmToken, driverId, role } = req.body;
    ...
    const requestedRole = role === 'driver' ? 'driver' : 'customer';
    ...
    // ✅ If not found, try by phone + role
    if (!user && phone) {
      const phoneKey = phone.replace(/^\+91/, "").replace(/^91/, "");
      user = await User.findOne({ phone: phoneKey, role: requestedRole });
      if (user) {
        console.log(`   ✅ Found user by phone+role: ${user.name} (${requestedRole})`);
      }
    }
...
router.post("/delete-fcm", async (req, res) => {
  try {
    const { driverId, phone, role } = req.body;
