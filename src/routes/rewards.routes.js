...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/rewards/debug/transactions/:customerId
// DEBUG: Check coin transactions for a user — verify spend records exist
// ─────────────────────────────────────────────────────────────────────────────
router.get('/debug/transactions/:customerId', async (req, res) => {
  try {
    const { customerId } = req.params;
    ...
    const user = await User.findById(customerId)
      .select('coins totalCoinsEarned totalCoinsRedeemed')
      .lean();
    ...
    res.json({
      ...
      totalRedeemed: user?.totalCoinsRedeemed || 0,
      transactionCount: transactions.length,
      transactions: transactions.map(tx => ({
        id: tx._id,
        type: tx.type,
        coins: tx.coinsEarned,
        description: tx.description,
        balanceAfter: tx.balanceAfter,
        createdAt: tx.createdAt,
        tripId: tx.tripId
...
router.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      'GET  /api/rewards/debug/transactions/:id — debug: user coin transactions',
    ],
  });
});
